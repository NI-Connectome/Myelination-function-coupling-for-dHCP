...
Gene_expr_pre = readmatrix('gene_expression.csv');
load('Gene_label.mat')
load('MFC.mat')  % the median value in 11 brain areas

genes=Gene_label; % this needs to be imported first
geneindex=1:length(Gene_label);
bootnum=5000;
X=zscore(log2(Gene_expr_pre'+1),0,1);
Y=zscore(MFC);
dim=10;
...
parfor i=1:bootnum
    ...
    Yr=Y(myresample,:); % define Y for resampled subjects
